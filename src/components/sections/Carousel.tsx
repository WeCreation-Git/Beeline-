...
const Carousel = () => {
  ...
  return (
    <div className="relative h-screen overflow-hidden">
      ...
      <button
        onClick={nextSlide}
        className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 hover:bg-opacity-70 text-yellow-400 p-3 rounded-full transition-all duration-300 border border-yellow-400"
      >
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            className={`w-4 h-4 rounded-full transition-all duration-300 border-2 ${
              index === currentSlide
                ...
                : "bg-transparent border-white hover:border-yellow-400"
            }`}
          />
        ))}
      </div>
